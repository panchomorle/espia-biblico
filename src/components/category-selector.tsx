...
function CategorySelector({ categories, setCategories }: Props) {
  ...
  return (
    ...
    <div className="w-full h-full flex flex-wrap justify-center p-4">
        ...
        { categories.map((c) => (
                <button key={c.name} className={`w-full sm:w-1/3 transition-all duration-200 ease-in ${c.enabled ? c.enabled_color : c.disabled_color}
            hover:border-4 hover:border-white focus:outline-none focus:ring-0`}
                onClick={()=> handleToggle(c.name)}>
                    ...
                    <label className={`font-bold ${!c.enabled ? "line-through": ""}`}>{c.name}</label>
                    </div>
                </button> 
        )
        )}
